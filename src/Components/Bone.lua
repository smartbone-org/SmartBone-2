...
local function SolveWind(self: IBone, BoneTree: any, Velocity: Vector3): Vector3
	...
	local TimeModifier = BoneTree.WindOffset
		+ (
			(
				...
				+ Magnitude(self.TransformOffset.Position - BoneTree.Root.WorldPosition) * 0.2
			) -- * 0.2 is / 5
	...
	if WindSpeed > 0 then
		if VelocityDirection:Dot(WindDirection) > 0 then -- Going with the wind
			local SpeedMatch = 1 - Magnitude(Velocity) / WindSpeed
	...
	local SpeedAlpha = Magnitude(Velocity) < 100 and Magnitude(Velocity) or 100
...
function Class:ApplyTransform(BoneTree)
	...
	-- if ParentBone.NumberOfChildren > 1 and Magnitude(ParentBone.RotationSum) ~= 0 then
